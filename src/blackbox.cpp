...
bool blackboxNeedsErase(const BlackboxState& state, uint32_t* eraseAddr) {
    ...
    if (state.sectorSize == 0) return false;
    ...
    if (nextSector != currentSector || state.writeOffset == 0) {
        if (eraseAddr) *eraseAddr = nextSector * state.sectorSize;
        return true;
    }
    return false;
}
...
int blackboxFormatRecord(const BlackboxRecord& rec, char* buf, size_t bufLen) {
    return snprintf(buf, bufLen,
        "%u,%.2f,%.4f,%.2f,%.1f,%u,%u,%u\r\n",
        rec.timestampMs,
        (double)rec.omega, (double)rec.phase,
        (double)rec.packVoltage, (double)rec.slipPct,
        rec.orientation, rec.hitDetected, rec.armState);
}
